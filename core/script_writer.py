...
if "text" is in {lang_name}. Do not include markdown fences, only the JSON array."""
    ...
    def write_script(self, topic: str, niche_label: str = "", language: str = "en",
        ...
        if scenes:
            ...
            # Persian script generated by a free OpenRouter model
            # (Llama 3.3 70B / GPT-OSS 120B / Qwen3, all documented via live
            # research to have measurably weak Arabic/Persian output
            # quality) came out partially incoherent -- the LLM call
            # "succeeded" from llm_router's point of view (got a 200 + valid
            # JSON back), so nothing before this caught it. Explicitly
            # validate that a script claiming to be Persian is ACTUALLY
            # written in Persian (not garbled/English-contaminated) before
            # accepting it -- reject and fall through to content_bank.py's
            # curated (verified-coherent) scripts otherwise, exactly like a
            # provider failure.
            validation = script_quality.validate_script_language(scenes, language)
            if not validation["ok"]:
                print(f"[ScriptWriter] Rejecting {engine} script -- failed language "
        ...
        if not scenes:
            ...
            # docs/LLM-PROVIDERS-2026.md), OR the LLM's output failed the
            # language-quality check above. Rather than immediately dropping
